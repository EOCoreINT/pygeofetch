...
rm = _load("pygeofetch.analysis.risk_mapping", "pygeofetch/insar/analysis.py")

# Realistic irregular acquisition gaps, matching this project's own
# real Mexico City stack.
DATES = ["2016-07-24", "2016-08-05", "2016-09-22", "2016-10-04", "2016-12-13", "2016-12-25"]
H, W = 6, 6
...
def test_resolve_time_years_matches_sbas_convention():
    print("=== 1. _resolve_time_years uses real elapsed time, matches SBASTimeSeries convention ===")
    t_years = rm._resolve_time_years(DATES)
    from datetime import datetime
    expected_days = [(datetime.strptime(d, "%Y-%m-%d") - datetime.strptime(DATES[0], "%Y-%m-%d")).days for d in DATES]
    ...
    assert np.allclose(t_years, expected_years), "time_years should match days/365.25 exactly"
...
def test_no_mutation_of_input_object():
    ...
    assert not hasattr(ts, "data"), "sanity check: fake object shouldn't have 'data' before construction"
    assert not hasattr(ts, "times"), "sanity check: fake object shouldn't have 'times' before construction"
    mapper = rm.RiskMapper(ts)
    assert not hasattr(ts, "data"), "ts_result should NOT have gained a 'data' attribute"
    assert not hasattr(ts, "times"), "ts_result should NOT have gained a 'times' attribute"
    assert mapper.data is not None
    assert mapper.time_years is not None
    print("  confirmed: ts_result unchanged after RiskMapper construction; resolved values live on the mapper")
    print("  PASS\n")


def test_default_risk_function_uses_real_time():
    print("=== 3. _default_risk_function recovers the true trend using real elapsed time ===")
    ts, t_years = make_synthetic_ts(velocity_mm_yr=TRUE_VELOCITY_MM_YR, noise_std_mm=0.2)
    mapper = rm.RiskMapper(ts)
    ...
    assert abs(recovered_mm_yr - TRUE_VELOCITY_MM_YR) < 2.0, "should recover the true trend within noise tolerance"
    print("  PASS\n")
...
def test_bayesian_prior_now_has_real_effect():
    ...
    s_weak, _ = mapper._bayesian_linear_regression(x, y, 5000, prior_mean=0.0, prior_std=1e6)
    ...
    s_strong, _ = mapper._bayesian_linear_regression(x, y, 5000, prior_mean=strong_prior_mean, prior_std=1e-4)
    print(f"  strong prior_mean: {strong_prior_mean:.6f}")
    print(f"  strong-prior posterior mean: {s_strong.mean():.6f} (should ~= prior_mean, NOT OLS)")
    assert abs(s_strong.mean() - strong_prior_mean) < 0.5, "a very confident prior should dominate the posterior"
    assert abs(s_strong.mean() - ols_slope) > 5.0, "posterior should have moved FAR from the OLS estimate"
    print("  PASS\n")


def test_bootstrap_no_longer_scrambles_time():
    print("=== 5. Residual bootstrap gives stable, sign-consistent slopes (not scrambled) ===")
    ts, t_years = make_synthetic_ts(velocity_mm_yr=TRUE_VELOCITY_MM_YR, noise_std_mm=0.3)
    mapper = rm.RiskMapper(ts)

    risk_map = mapper.compute_risk(method="bootstrap", confidence_level=0.95, n_simulations=200)
    print(f"  risk mean: {np.nanmean(risk_map.risk):.4f}")
    print(f"  uncertainty (std across bootstrap trials) mean: {np.nanmean(risk_map.uncertainty):.4f}")
    ...
    print(f"  coefficient of variation: {cv:.3f} (should be modest, not wildly unstable)")
    assert cv < 1.0, f"bootstrap uncertainty should be a reasonable fraction of the risk estimate, got CV={cv:.3f}"
    print("  PASS\n")
...
def test_export_uncertainty_handles_none_transform():
    ...
    assert risk_map.transform is None, "sanity check: FakeTimeSeriesResult has no transform"
...
def test_coverage_no_longer_fabricates_a_number():
    print("=== 7. validate_risk_map no longer echoes confidence_level as fake 'coverage' ===")
    ts, _ = make_synthetic_ts()
    mapper = rm.RiskMapper(ts)
    ...
    assert results["coverage"] is None, "coverage should be None (honest), not silently equal to confidence_level"
    print("  PASS\n")


def test_plot_risk_map_renders():
    print("=== 8. plot_risk_map (including uncertainty hatching) renders without error ===")
    import tempfile
    ts, _ = make_synthetic_ts()
    mapper = rm.RiskMapper(ts)
