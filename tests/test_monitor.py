...
def test_new_date_connects_to_n_nearest_existing_neighbors():
    print(
        "=== 1. A single new date connects to its N nearest chronological neighbors, not the full archive ==="
    )
    ...
    ), f"expected {expected_neighbors}, got {actual_neighbors}"
    assert (
        len(pairs) == 3
    ), "should form exactly 3 new pairs (n_neighbors=3), not the full archive"
    print(
        "  PASS -- confirms this does NOT reprocess the whole archive, matching the spec's real intent\n"
    )


def test_batch_of_new_dates_connects_to_each_other_too():
    print(
        "=== 2. A BATCH of new dates arriving together correctly connects to each other, not just the old archive ==="
    )
    existing = ["2024-01-01", "2024-01-13"]
    new = [
        "2024-02-01",
        "2024-02-13",
    ]  # two new dates, close to each other, far from existing
    ...
    assert (
        "2024-02-01",
        "2024-02-13",
    ) in pairs, "two new dates close together must connect to each other, not just to the distant old archive"
    print(
        "  PASS -- confirms a real batch of new scenes forms a real, connected sub-network, not isolated spokes\n"
    )


def test_no_new_dates_plans_nothing():
    print(
        "=== 3. plan_monitoring_run correctly plans NOTHING when there are genuinely no new dates ==="
    )
    with tempfile.TemporaryDirectory() as d:
        state = ProjectState(Path(d) / "state.db")
        state.mark_dates_processed(["2024-01-01", "2024-01-13"])

        result = plan_monitoring_run(
            state, available_dates=["2024-01-01", "2024-01-13"]
        )
...
def test_running_twice_with_no_new_scenes_is_idempotent():
    print(
        "=== 4. Real idempotency, matching the spec's explicit requirement: running twice on the same day does the same nothing both times ==="
    )
    with tempfile.TemporaryDirectory() as d:
        state = ProjectState(Path(d) / "state.db")
        ...
        # on the same day.
        result_1 = plan_monitoring_run(
            state, available_dates=["2024-01-01", "2024-01-13", "2024-01-25"]
        )
        result_2 = plan_monitoring_run(
            state, available_dates=["2024-01-01", "2024-01-13", "2024-01-25"]
        )
...
def test_plan_correctly_detects_real_new_dates():
    print(
        "=== 5. plan_monitoring_run correctly identifies genuinely new dates against real persisted state ==="
    )
    with tempfile.TemporaryDirectory() as d:
        state = ProjectState(Path(d) / "state.db")
        state.mark_dates_processed(["2024-01-01", "2024-01-13"])
