...
class TestDataValidator:
    ...
    def test_sbas_network_connected_passes(self):
        ...
        pairs = [
            InterferogramPair(dates[0], dates[1], np.zeros((4, 4)), np.ones((4, 4)) * 0.8),
            InterferogramPair(dates[1], dates[2], np.zeros((4, 4)), np.ones((4, 4)) * 0.8),
        ]
    ...
    def test_sbas_network_disconnected_rejected(self):
        ...
        pairs = [
            InterferogramPair(dates[0], dates[1], np.zeros((4, 4)), np.ones((4, 4)) * 0.8),
            InterferogramPair(dates[2], dates[3], np.zeros((4, 4)), np.ones((4, 4)) * 0.8),
        ]
        result = DataValidator.validate_sbas_network(pairs, dates)
        assert not result.valid
    ...
    def test_sbas_network_uses_real_interferogrampair_attribute_names(self):
        ...
        pairs = [InterferogramPair(dates[0], dates[1], np.zeros((4, 4)), np.ones((4, 4)) * 0.8)]
        result = DataValidator.validate_sbas_network(pairs, dates)
        assert result.valid
...
<numberOfLines>{n_lines}</numberOfLines><numberOfColumns>{n_cols}</numberOfColumns>
