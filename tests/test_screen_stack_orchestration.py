...
def make_geom(scene_center_time, n_lines=2000):
    ...
    return annotation.SLCGeometry(
        first_line_time=scene_center_time
        - timedelta(seconds=(n_lines / 2) * az_interval),
        azimuth_time_interval_s=az_interval,
        near_range_time_s=0.005,
        range_sampling_rate_hz=6.0e7,
        n_lines=n_lines,
        n_columns=8000,
    )
...
FAKE_BURSTS = {
    d: make_burst_info(NOMINAL[d] - timedelta(seconds=1.5)) for d in CROSSING
}
...
if __name__ == "__main__":
    ...
    with (
        patch.object(
            annotation, "parse_slc_geometry", side_effect=fake_parse_slc_geometry
        ),
        patch.object(
            geolocation, "parse_orbit_file", side_effect=fake_parse_orbit_file
        ),
    ):
        # annotation.parse_burst_info patched separately since it's a
        # distinct real function in the same module.
        with patch.object(
            annotation, "parse_burst_info", side_effect=fake_parse_burst_info
        ):
    ...
    print(
        f"Screened {len(results)} pairs from {len(dates)} dates (expected {len(dates)*(len(dates)-1)//2})"
    )
    ...
    for (d1, d2), r in by_pair.items():
        print(
            f"  {d1} -> {d2}: Δt_acq={r.sync_offset_ms:+.3f}ms, within={r.within_requirement}"
        )
    ...
    for (d1, d2), r in by_pair.items():
        ...
        assert (
            r.within_requirement != expect_bad
    ...
    assert (
        len(results2) == 6
    ), f"orphan date should be silently skipped, expected 6 pairs, got {len(results2)}"
