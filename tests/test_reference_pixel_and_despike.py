...
def test_reference_pixel_matches_bridge_unwrap_regions_criterion():
    print(
        "=== 0. Real bug regression: conncomp>0 is not enough, must match "
...
def test_relaxes_gracefully_when_no_perfect_pixel_exists():
    print(
        "=== 2. No pixel reliable everywhere -> graceful relaxation, honest report ==="
    )
    h, w = 10, 10
    ...
    pixel, report = select_reliable_reference_pixel(
        conncomp, min_reliable_fraction=1.0, min_region_size=1
    )
    print(
        f"  chosen pixel: {pixel}, reliable_fraction={report['reliable_fraction']:.3f}, "
...
def test_prefers_real_landmark_when_equally_good():
    print(
        "=== 3. Preferred (real, georeferenced) point used when it's just as good ==="
    )
    h, w = 15, 15
    pairs = [("d1", "d2"), ("d1", "d3")]
    conncomp = {p: np.zeros((h, w), dtype=np.int32) for p in pairs}
    ...
    print(
        f"  chosen pixel: {pixel}, searched_near_preferred={report['searched_near_preferred']}"
    )
    ...
    print(
        f"  chosen pixel: {pixel2}, searched_near_preferred={report2['searched_near_preferred']}"
    )
...
def test_despike_removes_isolated_spike():
    ...
    assert (
        abs(filtered[15, 15] - background[15, 15]) < 1.0
    ), "spike should be removed, close to real background"
...
def test_despike_nan_handling():
    ...
    for v in neighbour_vals:
        assert (
            abs(v - 2.0) < 1e-6
        ), f"neighbour of a NaN pixel should be unaffected, got {v}"
