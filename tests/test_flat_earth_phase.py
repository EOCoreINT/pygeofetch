...
def _build_fixture(n=200, b_perp=80.0):
    sat_pos_ref, sat_vel_ref, range_time_center, P_center = _build_geometry(
        19.36, -99.09
    )
...
def test_range_difference_matches_textbook_approximation():
    ...
    def make_simple_orbit(baseline_offset_m):
        times = [
            datetime(2024, 1, 1) + timedelta(seconds=t)
            for t in np.linspace(-10, 10, 200)
        ]
        positions = [
            (t * 7500.0, baseline_offset_m, EARTH_R + alt)
            for t in np.linspace(-10, 10, 200)
        ]
    ...
    t_ref = find_zero_doppler_time(
        ref_orbit[0], ref_orbit[1], ref_orbit[2], ground_point, scene_center_time
    )
    t_sec = find_zero_doppler_time(
        sec_orbit[0], sec_orbit[1], sec_orbit[2], ground_point, scene_center_time
    )
...
def test_matches_independent_direct_computation_at_a_real_point():
    ...
    flat_phase = compute_flat_earth_phase(
        ...
        (n, n),
        WAVELENGTH_M,
        sample_bounds=(lon0, lat0, lon0 + 0.04, lat0 + 0.04),
        grid_points=7,
        polynomial_degree=2,
    )
    ...
    t_ref = find_zero_doppler_time(
        ref_orbit[0], ref_orbit[1], ref_orbit[2], ground_point, t0_ref
    )
    t_sec = find_zero_doppler_time(
        sec_orbit[0], sec_orbit[1], sec_orbit[2], ground_point, t0_sec
    )
    pos_ref, _ = interpolate_orbit_state(*ref_orbit, t_ref)
    pos_sec, _ = interpolate_orbit_state(*sec_orbit, t_sec)
    R_ref = sum((pos_ref[i] - ground_point[i]) ** 2 for i in range(3)) ** 0.5
...
def test_closed_loop_removal_through_complex_wrapped_pathway():
    ...
    true_deformation_phase = 0.5 * np.exp(
        -((np.arange(n)[:, None] - n / 2) ** 2 + (np.arange(n)[None, :] - n / 2) ** 2)
        / (n * 2)
    )
