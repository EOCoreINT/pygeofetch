...
def test_reproduces_real_bad_outcome():
    print(
        "=== 1. Confirm old baseline-only algorithm reproduces the real bad outcome ==="
    )
    ...
    print(
        f"  old network: {len(old_selected)} pairs, {cross_family} cross-family, mean coherence {mean_coh:.3f}"
    )
    for d1, d2, c in old_selected:
        print(
            f"    {d1} -> {d2}: coherence={c:.3f} ({'cross' if FAMILY[d1]!=FAMILY[d2] else 'same'}-family)"
        )
    assert (
        cross_family >= 3
    ), "should reproduce the real bad outcome (mostly cross-family pairs)"
...
def test_build_sbas_network_fixes_it():
    ...
    selected, report = build_sbas_network(
        candidates, DATES, min_coherence=0.3, redundancy=2
    )
    ...
    print(
        f"  selected: {len(selected)} pairs ({len(report['good_pairs'])} good, "
        f"{len(report['bridge_pairs'])} bridge)"
    )
    for d1, d2 in selected:
        tag = "BRIDGE" if (d1, d2) in report["bridge_pairs"] else "good"
        print(
            f"    {d1} -> {d2}: coherence={coh_by_pair[(d1,d2)]:.3f} [{tag}, "
            f"{'cross' if FAMILY[d1]!=FAMILY[d2] else 'same'}-family]"
        )
    print(
        f"  connected: {report['connected']}, unconnected dates: {report['unconnected_dates']}"
    )
    ...
    assert (
        mean_coh > 0.40
    ), f"expected substantially better mean coherence, got {mean_coh:.3f}"
    ...
    assert (
        min(degree.values()) >= 2
    ), "every date should have redundant connections, not just one"
    ...
    assert (
        mean_coh > old_mean + 0.10
    ), "new network should be substantially better than the old one"
    print("  PASS\n")


def test_no_good_bridge_available():
    print(
        "=== 3. Graceful degradation: no candidate pair connects an isolated date ==="
    )
    candidates = [
        PairCandidate(d1, d2, perpendicular_baseline_m=b, coherence=c)
        for d1, d2, b, c in REAL_PAIRS
    ]
    dates_plus_orphan = DATES + [
        "2016-10-05"
    ]  # no candidate pair involves this date at all
    selected, report = build_sbas_network(
        candidates, dates_plus_orphan, min_coherence=0.3, redundancy=2
    )
    print(
    ...
    print(
        "  correctly reports the orphan date instead of silently dropping or crashing"
    )
    print("  PASS\n")
