...
def test_adi_correctly_ranks_stable_vs_noisy_pixels():
    print(
        "=== 1. ADI is low for a genuinely stable pixel, high for a genuinely noisy one ==="
    )
    ...
    assert (
        adi[0, 0] < 0.25
    ), "a genuinely stable point target should clear the strict ADI threshold"
    assert (
        adi[1, 1] > 0.40
    ), "a genuinely noisy distributed scatterer should fail even the relaxed threshold"
    print("  PASS\n")


def test_selection_correctly_separates_real_ps_from_bright_noisy_and_dim_stable():
    print(
        "=== 2. select_persistent_scatterers correctly picks the real PS pixel, rejecting both a bright-but-noisy and a dim-but-stable one ==="
    )
    ...
    stack = rng.normal(
        60.0, 15.0, (n, h, w)
    )  # ordinary background pixels, real but unremarkable
    stack[:, 0, 0] = rng.normal(150.0, 3.0, n)  # real PS: bright AND stable
    stack[:, 0, 1] = rng.normal(
        150.0, 90.0, n
    )  # bright but genuinely noisy -- should be rejected
    ...
    stack[:, 0, 2] = rng.normal(
        5.0, 0.1, n
    )  # very stable but genuinely dim -- should fail the amplitude floor
    ...
    assert not result.ps_mask[
        0, 1
    ], "the bright-but-noisy pixel should be rejected on ADI"
    assert not result.ps_mask[
        0, 2
    ], "the dim-but-stable pixel should be rejected on the amplitude floor"
    print(
        "  PASS -- confirms BOTH real criteria (ADI ceiling AND amplitude floor) are actually enforced, not just one\n"
    )


def test_empty_selection_reported_honestly_not_raised():
    print(
        "=== 3. Zero real candidates is reported honestly (n_candidates=0), not raised as an exception ==="
    )
    ...
    print(
        "  PASS -- matches this project's 'fail loudly, never silently interpolate' principle: honestly empty, not an error\n"
    )


def test_temporal_coherence_high_for_consistent_residuals_low_for_random():
    print(
        "=== 4. Real temporal coherence is high when residuals are consistently near zero, low when they're effectively random ==="
    )
    wavelength_m = 0.0555
    n_pairs, h, w = 15, 1, 2
    ...
    residuals[:, 0, 0] = np.random.default_rng(4).normal(
        0, 0.0005, n_pairs
    )  # ~0.5mm real residual noise
    ...
    assert (
        gamma[0, 0] > 0.95
    ), "small, consistent residuals should give real temporal coherence near 1"
    assert (
        gamma[0, 1] < 0.5
    ), "effectively random residuals should give real temporal coherence well below the spec's 0.7 threshold"
    print("  PASS\n")


def test_refine_correctly_demotes_ps_candidate_with_poor_temporal_coherence():
    print(
        "=== 5. A real ADI-selected candidate with poor temporal coherence is correctly demoted, not kept just because it passed amplitude screening ==="
    )
    ...
    amp_stack[:, 0, 0] = rng.normal(
        150.0, 2.0, n_amp
    )  # stable amplitude -- real ADI candidate
    amp_stack[:, 0, 1] = rng.normal(
        150.0, 2.0, n_amp
    )  # ALSO a real ADI candidate by amplitude alone

    ps_result = ps_mod.select_persistent_scatterers(
        amp_stack, amplitude_percentile=70.0
    )
    assert (
        ps_result.ps_mask[0, 0] and ps_result.ps_mask[0, 1]
    ), "both should pass the amplitude-only screening"

    n_pairs = 15
    residuals = np.zeros((n_pairs, h, w))
    residuals[:, 0, 0] = rng.normal(
        0, 0.0005, n_pairs
    )  # pixel 0: genuinely fits the model well
    residuals[:, 0, 1] = (
        rng.uniform(-wavelength_m / 8, wavelength_m / 8, n_pairs) * 4
    )  # pixel 1: poor real fit

    refined = ps_mod.refine_ps_mask_with_temporal_coherence(
        ps_result, residuals, wavelength_m, coherence_threshold=0.7
    )
    print(f"  before refinement: {ps_result.ps_mask[0]}")
    print(f"  after refinement:  {refined.ps_mask[0]}")
    assert refined.ps_mask[
        0, 0
    ], "the pixel with real, consistent residuals should survive refinement"
    assert not refined.ps_mask[
        0, 1
    ], "the pixel with poor real temporal coherence should be demoted"
    print(
        "  PASS -- confirms the AND is real: amplitude-only selection alone is not enough to survive refinement\n"
    )
...
def test_aps_recovers_known_smooth_pattern_from_sparse_ps_pixels():
    print(
        "=== 6. APS estimation recovers a known, smooth atmospheric pattern from genuinely SPARSE PS coverage ==="
    )
    h, w = 200, 200
    ...
    true_aps = (
        0.01 * xx
        + 0.005 * yy
        + 0.02 * np.exp(-((xx - 100) ** 2 + (yy - 100) ** 2) / (2 * 60**2))
    )
    ...
    print(
        f"  real PS coverage: {ps_mask.sum()}/{h*w} pixels ({100*ps_mask.sum()/(h*w):.2f}%)"
    )

    per_date_values = np.where(ps_mask, true_aps, np.nan)

    estimated_aps = ps_mod.estimate_atmospheric_phase_screen(
        per_date_values, ps_mask, spatial_filter_sigma_px=20.0
    )
    ...
    print(
        f"  mean abs error: {error.mean():.5f} ({100*relative_error:.2f}% of signal span)"
    )
    ...
    assert (
        relative_error < 0.02
    ), f"mean APS recovery relative error too large: {100*relative_error:.2f}%"
    print(
        "  PASS -- confirms the real, smooth atmospheric pattern is recovered from realistically sparse PS coverage\n"
    )


def test_aps_regions_with_no_real_ps_coverage_return_nan_not_zero():
    print(
        "=== 7. A real region with genuinely zero PS coverage nearby returns NaN, not a silently wrong zero ==="
    )
    h, w = 100, 100
    ...
    aps = ps_mod.estimate_atmospheric_phase_screen(
        values, ps_mask, spatial_filter_sigma_px=5.0
    )
    print(f"  APS near the one real PS pixel: {aps[10,10]:.4f}")
    print(f"  APS far from any real PS pixel: {aps[90,90]}")
    assert not np.isnan(
        aps[10, 10]
    ), "right at a real PS pixel, an estimate should exist"
    assert np.isnan(
        aps[90, 90]
    ), "far from any real PS coverage, this must be honestly NaN, not a fabricated zero"
    print("  PASS\n")
