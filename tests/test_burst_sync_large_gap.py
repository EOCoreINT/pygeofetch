...
def build_orbit(true_crossing_time: datetime, window_s=30):
    ...
    times = [
        true_crossing_time + timedelta(seconds=float(s))
...
def build_swath_timing(first_burst_time: datetime, n_bursts=8, cycle_s=CYCLE_S):
    ...
    return annotation.SwathTiming(
        lines_per_burst=1506, samples_per_burst=23674, bursts=bursts
    )
...
if __name__ == "__main__":
    ...
    T_SEC = T_REF + timedelta(
        days=GAP_DAYS
    )  # clean gap; residual injected via burst phase below

    print(f"Reference crossing: {T_REF}")
    print(f"Secondary crossing: {T_SEC}  (exactly {GAP_DAYS} days later)")
    print(
        f"Real burst cycles spanned: {(T_SEC - T_REF).total_seconds() / CYCLE_S:,.0f}"
    )
    print()
    ...
    sec_burst_info = build_swath_timing(
        T_SEC - timedelta(seconds=1.5 - KNOWN_RESIDUAL_MS / 1000.0),
        cycle_s=CYCLE_S + 3e-7,
    )

    print("=== OLD (buggy) formula: raw dt modulo one averaged cycle ===")
    old_result_ms, cycle_used = old_buggy_formula(
        T_REF, T_SEC, ref_burst_info, sec_burst_info
    )
    print(
        f"  cycle estimate used: {cycle_used:.9f}s (true: {CYCLE_S:.9f}s, error: {cycle_used-CYCLE_S:.2e}s)"
    )
    ...
    result = esd.compute_burst_synchronization(
        ...
        sec_burst_info,
        GROUND_POINT,
        ref_time_guess=T_REF,
        sec_time_guess=T_SEC,
    )
    ...
    print(
        f"OLD formula: {old_error:.1f}ms error from a {cycle_used-CYCLE_S:.2e}s cycle estimation "
    ...
    assert (
        new_error < 0.5
    ), f"NEW formula should recover the true residual accurately, got error={new_error:.4f}ms"
    print(
        f"NEW formula: {new_error:.4f}ms error -- recovers the true {KNOWN_RESIDUAL_MS}ms residual "
