...
def test_reproduces_mexico_city_duplicate_slice_fix():
    ...
    assert (
        report["picked_non_first"] == 1
    ), "should detect the one date that needed the fix"
    ...
    assert kept_2016_11_09.geometry == poly(
        0, 0
    ), "should have kept the COVERING slice, not the first-arriving one"
    print("  PASS\n")
...
def test_reproduces_obuasi_truncation_warning():
    ...
    scenes = [
        FakeScene(d0 + timedelta(days=12 * i), poly(0, 0), track=45) for i in range(5)
    ]
...
def test_zero_result_search_raises_clearly():
    print(
        "=== 3. Empty search raises a clear, honest error, not a downstream crash ==="
    )
    ...
    try:
        stack_selection.search_and_select_consistent_stack(
            FakeClient(), AOI, "2019-01-01", "2019-06-01"
        )
...
def test_final_coverage_verification_drops_genuine_gaps():
    print(
        "=== 4. Final verification drops a genuine gap even after coverage-aware dedup ==="
    )
    d0 = datetime(2020, 1, 1)
    scenes = [
        FakeScene(d0, poly(0, 0), track=45),  # good
        FakeScene(
            d0 + timedelta(days=12), poly(50, 50), track=45
        ),  # ONLY candidate for this date, and it's bad
    ...
    assert (
        "2020-01-13" not in dates
    ), "the genuinely non-covering date should be dropped"
    assert len(dates) == 2
    print("  PASS\n")
...
def test_burst_family_classification_majority_exclusive():
    print(
        "=== 5. select_burst_synchronized_dates: majority family used exclusively when large enough ==="
    )
    ...
    assert set(chosen) == set(
        report["good_dates"]
    ), "minority dates should be excluded entirely"
    print("  PASS\n")


def test_burst_family_classification_keeps_bridges_when_majority_too_small():
    print(
        "=== 6. select_burst_synchronized_dates: keeps bridges when majority family is too small ==="
    )
    ...
    assert set(chosen) == {
        ...
        "d3",
        "d4",
    }, "minority date(s) should be kept as bridges"
    print("  PASS\n")
