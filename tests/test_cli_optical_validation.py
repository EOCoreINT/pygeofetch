...
def make_scene(scene_id, bands=None, cloud_cover=5.0):
    ...
    return SatelliteData(
        id=scene_id, provider="aws_earth", bbox=(-74.2, 40.5, -73.6, 41.0),
        cloud_cover=cloud_cover, processing_level=ProcessingLevel.L2A,
        assets={b: SatelliteAsset(key=b, href=f"https://x/{b}.tif", roles=["data"]) for b in bands},
    )
...
class TestSearchRunValidateOptical:
    ...
    def test_missing_bands_scene_filtered_out(self):
        ...
        runner = CliRunner()
        with patch("pygeofetch.core.searcher.FederatedSearcher.search", return_value=[good, bad]):
            result = runner.invoke(cli, [
                "search", "run", "--bbox", "-74.1,40.6,-73.7,40.9",
                "--validate-optical", "--format", "ids",
            ])
        assert result.exit_code == 0
    ...
    def test_without_flag_all_results_returned(self):
        ...
        with patch("pygeofetch.core.searcher.FederatedSearcher.search", return_value=[good, bad]):
            result = runner.invoke(cli, [
                "search", "run", "--bbox", "-74.1,40.6,-73.7,40.9", "--format", "ids",
            ])
        assert result.exit_code == 0
    ...
    def test_optical_threshold_override_applied(self):
        ...
        with patch("pygeofetch.core.searcher.FederatedSearcher.search", return_value=[cloudy]):
        ...
        with patch("pygeofetch.core.searcher.FederatedSearcher.search", return_value=[cloudy]):
            strict_result = runner.invoke(cli, [
                "search", "run", "--bbox", "-74.1,40.6,-73.7,40.9",
