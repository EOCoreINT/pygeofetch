...
class SARProcessor:
    ...
    def coherence(
        ...
        Real, consolidated implementation: this now calls the shared,
        canonical formula in pygeofetch.utils.sar_math (see that
        module's docstring for why this used to be a second, separate
        copy of the same real formula also present in
        pygeofetch.insar.interferogram, and why only this standalone
        copy -- not that one -- was safe to consolidate).
        ...
        # Read — complex GeoTIFFs report dtype complex64. Real,
        # confirmed bug fixed here: _safe_read_band always casts to
        # real float32 (its own docstring says so explicitly), which
        # silently discards the imaginary part *before* the .view()
        # below ever runs -- meaning .view(complex64) was reinterpreting
        # already-real-valued memory, not recovering genuine phase data.
        # Confirmed directly: this made coherence() non-functional for
        # its entire stated purpose (real complex SLC input) -- caught
        # by a real test using an actual synthetic complex64 GeoTIFF,
        # not assumed from reading the code. Fixed by reading complex
        # bands directly via rasterio, never through the lossy helper.
        with rasterio.open(inp1) as src:
            profile = src.profile.copy()
            if src.dtypes[0].startswith("complex"):
                s1 = src.read(1)
            else:
        ...
        with rasterio.open(inp2) as src2:
            if src2.dtypes[0].startswith("complex"):
                s2 = src2.read(1)
                if s2.shape != s1.shape:
                    raise ValueError(
                        f"coherence: complex SLC shape mismatch {s1.shape} vs "
                        f"{s2.shape} -- co-register onto the same real grid first "
                        f"(resampling a complex raster would corrupt its phase)."
                    )
            else:
        ...
        coh = estimate_interferometric_coherence(s1, s2, window=window)
