...
SNAPHU_DIR = Path(
    os.environ.get("PYGEOFETCH_SNAPHU_DIR", Path.home() / ".pygeofetch" / "bin")
)
...
class UnwrapResult:
    ...
    def save(
        ...
        base_profile = {
            "driver": "GTiff",
            "count": 1,
            "height": self.unwrapped_phase.shape[0],
            "width": self.unwrapped_phase.shape[1],
        ...
        logger.info(
            "Unwrap products saved → %s (reliable=%.1f%%)",
            out_dir,
            self.reliable_fraction * 100,
        )
        if save_png:
...
def _verify_sha256(filepath: Path, expected_hash: Optional[str]) -> bool:
    if not expected_hash:
        logger.warning(
            "No SHA256 hash provided for %s. Skipping verification.", filepath.name
        )
        return True
...
def _check_url_alive(url: str) -> bool:
    try:
        req = urllib.request.Request(
            url, method="HEAD", headers={"User-Agent": "PyGeoFetch/1.0"}
        )
...
def _ensure_snaphu_cli() -> str:
    ...
    if platform.system() == "Windows":
        logger.info(
            "SNAPHU executable not found or outdated. Downloading pre-compiled binary..."
        )
            ...
            try:
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    if not _verify_sha256(zip_path, KNOWN_HASHES.get(Path(url).name)):
                        raise ValueError(
                            "Checksum verification failed! File may be corrupted or tampered with."
                        )
                    ...
                    with zipfile.ZipFile(zip_path, "r") as zf:
                        ...
                        exe_files = [
                            ...
                            if f.lower().endswith(".exe") and "snaphu" in f.lower()
                        ]
                        if not exe_files:
                            exe_files = [
                                f for f in zf.namelist() if f.lower().endswith(".exe")
                            ]
                        if not exe_files:
                            raise RuntimeError(
                                "Downloaded zip did not contain any .exe file."
                            )
        ...
        raise RuntimeError(
            "Failed to download/extract SNAPHU executable from all sources."
        )
...
def goldstein_filter(
    interferogram, alpha: float = 0.5, tile_size: int = 32, overlap: float = 0.5
):
    import numpy as np
...
def bridge_unwrap_regions(
    ...
    if reference_pixel is not None:
        ...
        if rp_label == 0 or rp_label not in valid_labels:
            raise ValueError(
                f"reference_pixel {reference_pixel} is not part of any valid region."
            )
    ...
    resolved_points = {
        reference_label: np.column_stack(np.where(conncomp == reference_label))
    }
    ...
    while remaining:
        ...
        def _local_median(point, region_mask):
            ...
            return (
                float(np.median(values)) if len(values) > 0 else float(corrected[r, c])
            )
...
def multilook(
    ...
    if wrapped_phase is None:
        raise ValueError(
            "multilook() received real-valued input but wrapped_phase was not specified."
        )
    if wrapped_phase:
...
"""
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, cwd=str(tmp), timeout=3600
            )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if result.returncode in (3221225781, -1073741515):
                logger.warning(
                    "SNAPHU crashed with STATUS_DLL_NOT_FOUND (0xC0000135). Auto-bundling runtime DLLs..."
                )
            ...
            if result.returncode != 0:
                raise RuntimeError(
                    f"SNAPHU CLI failed with code {result.returncode}: {result.stderr}"
                )
            ...
            if conncomp.size < expected_pixels:
                logger.warning(
                    ...
                    "(known Windows port truncation quirk).",
                    conncomp.size,
                    expected_pixels,
                )
                conncomp = np.pad(
                    conncomp,
                    (0, expected_pixels - conncomp.size),
                    mode="constant",
                    constant_values=0,
                )
    ...
    def _ensure_runtime_dlls(self, bin_dir: Path):
        ...
        if len(found_dlls) >= len(
            [d for d in missing if d.startswith("lib") or d.startswith("msys")]
        ):
            logger.info(
                "Found MinGW/MSYS DLLs in Git for Windows installation. Copying to SNAPHU bin..."
            )
        ...
        try:
            req = urllib.request.Request(
                nuget_url, headers={"User-Agent": "PyGeoFetch"}
            )
                ...
                for name in zf.namelist():
                    if "win-x64/native/" in name and name.lower().endswith(".dll"):
                        ...
                        if dll_name.lower() in [m.lower() for m in missing]:
                            with (
                                zf.open(name) as src,
                                open(bin_dir / dll_name, "wb") as dst,
                            ):
                                shutil.copyfileobj(src, dst)
            logger.info(
                f"Successfully extracted MSVC DLLs to {bin_dir}. Retrying SNAPHU..."
            )
            return
        except Exception as e:
        ...
        if _check_url_alive(esa_url):
            logger.info(
                "Attempting to download self-contained ESA STEP SNAPHU build..."
            )
            try:
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    with zipfile.ZipFile(zip_path, "r") as zf:
                        for file_info in zf.infolist():
                            ...
                            with (
                                ...
                                shutil.copyfileobj(src, dst)
                    logger.info(
                        "Successfully installed self-contained ESA SNAPHU build. Retrying..."
                    )
                    return
            except Exception as e:
    ...
    def unwrap(
        ...
        try:
            ...
            logger.info(
                "snaphu Python package not found. Falling back to SNAPHU CLI executable..."
            )
            unwrapped, conncomp = self._unwrap_cli(
                igram, corr, nlooks, min_conncomp_frac, min_region_size
            )
        ...
        if pct > 30:
            logger.warning(
                "%.1f%% of pixels are in the unreliable connected component (conncomp==0).",
                pct,
            )
        else:
    ...
    def unwrap_pair(
        ...
        unwrapped, conncomp = self.unwrap(
            interferogram,
            coherence,
            nlooks=nlooks,
            mask=mask,
        ...
        if looks_azimuth > 1 or looks_range > 1:
            if scaled_profile.get("transform") is not None:
                ...
                scaled_profile["transform"] = scaled_profile[
                    "transform"
                ] * Affine.scale(looks_range, looks_azimuth)
...
def verify_snaphu_ready():
    ...
    if importlib.util.find_spec("snaphu") is not None:
        msg = (
            "✅ Success: snaphu Python module detected. No manual installation needed."
        )
        print(msg)
        logger.info(msg)
        return True
    ...
    msg = (
        "❌ snaphu not found in PATH or Python environment. Phase unwrapping will fail."
    )
    print(msg)
    logger.warning(msg)
    return False
