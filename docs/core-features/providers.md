...
|---|---|---|---|
| `usgs` | 🔐 Username + M2M Application Token | 🟢 Verified | Landsat 1–9, MODIS, EO-1, and 500+ other datasets via the real USGS Machine-to-Machine (M2M) API. Real, current, time-sensitive detail: USGS deprecated password-based login for a required **M2M Application Token** on February 26, 2025 — confirmed directly against USGS's own official documentation ("authenticate with this token *instead of* your ERS password") and multiple real downstream projects that broke on that exact date. The token *replaces* the password; it is not used alongside it. Two real, separate steps, in order: (1) request M2M access for your account at `ers.cr.usgs.gov/profile/access` (select "MACHINE" access type; real accounts report a couple of days for approval) — a token generated before this step is approved won't work; (2) once approved, generate a token from your ERS profile page ("Application Token"). |
