...
:::{admonition} Deduplication is only provider+ID based
:class: warning
...
:::
:::{admonition} Scoring is 3 factors, not the 4 the docstring claims
:class: note
...
*is* available separately via [Optical Data Validation & Preflight](optical-validation.md)'s
...
| `--validate-optical` | flag | Run optical preflight validation on results before returning them (AOI coverage, cloud cover, required bands, processing level, temporal bounds). See [Optical Data Validation & Preflight](optical-validation.md). Not appropriate for SAR/InSAR searches. |
...
Full `SearchQuery` field reference: [Python API Reference](../reference/python-api.md).
