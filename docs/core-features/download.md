...
| `--validate-optical` | flag | Run optical preflight validation as a final gate before downloading. Rejected items are never attempted -- a `FAILED` result explaining why is returned in their place, at their original position, so output length always matches input. See [Optical Data Validation & Preflight](optical-validation.md). |
