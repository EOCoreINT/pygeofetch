...
| `KeyringUnavailableError` | Set `PYTHON_KEYRING_BACKEND=keyring.backends.null.Keyring`. No keyring daemon (Docker, headless SSH) — use env vars, or the file backend, which now genuinely encrypts stored credentials (see [Security Model](security.md)). |
